...
Network::Network( int val, int postContScen, int scenarioContingency, int lineOutaged, int prePostScenario, int solverChoice, int dummy, int accuracy, int intervalNum, int lasIntFlag, int nextChoice, int outagedLine )
	: networkID( val ), // constructor begins; initialize networkID  and Rho through constructor initializer list
	  Rho( 1.0 ),
	  scenarioIndex( scenarioContingency ), // this is always zero for a base-case network instance, even if that corresponds to an outaged base case, or in other words, if postContScen is not zero
	  postContScenario( postContScen ),
...
void Network::setNetworkVariables( int networkID, int nextChoice ) // Function setNetworkVariables starts to initialize the parameters and variables
{
	...
	do {
		...
		for ( int l = 0; l < nodeNumber; ++l ) {
			...
			int modifiedContCount; // modified contingency count, to account for the change in contingency scenarios in different post-cont scenarios
			if (postContScenario==0) // for no outage case
				modifiedContCount = contingencyCount;
			else // for the outaged cases
				modifiedContCount = contingencyCount-1;
		
			Node nodeInstance( l + 1, modifiedContCount ); // creates nodeInstance object with ID l + 1
		...
		for ( int k = 0; k < translNumber; ++k ) {
			...
			if ((OutagedLine != (k+1)) && (baseOutagedLine != (k+1))){
		...
		for ( int i = 0; i < genNumber; ++i ) {
			...
			GensolverFirstDZ genParamDZBase( contingencyCount, c2, c1, c0, PgMax, PgMin, RgMax, RgMin, Beta, innerBeta, externalGamma, Gamma ); // Instantiate the copy constructor for the generator solver object
			GensolverFirst genParamFirst( contingencyCount, c2, c1, c0, PgMax, PgMin, RgMax, RgMin, Beta, innerBeta, externalGamma, Gamma, PgPrevious ); // Instantiate the copy constructor for the generator solver object
			GensolverSecondBase genParamSecondBase( contingencyCount, c2, c1, c0, PgMax, PgMin, RgMax, RgMin, Beta, innerBeta, externalGamma, Gamma, nextChoice ); // Instantiate the copy constructor for the generator solver object
			GensolverFirstCont genParamFirstCont( contingencyCount, c2, c1, c0, PgMax, PgMin, RgMax, RgMin, Beta, externalGamma, innerBeta, Gamma ); // Instantiate the copy constructor for the generator solver object
			GensolverFirstDZCont genParamFirstDZCont( contingencyCount, c2, c1, c0, PgMax, PgMin, RgMax, RgMin, Beta, externalGamma, innerBeta, Gamma ); // Instantiate the copy constructor for the generator solver object
			GensolverSecondCont genParamSecondCont( contingencyCount, c2, c1, c0, PgMax, PgMin, Beta, externalGamma, innerBeta, Gamma, RgMax, RgMin, nextChoice ); // Instantiate the copy constructor for the generator solver object
			...
			Generator generatorInstance( i + 1, intervalID, lastFlag, scenarioIndex, postContScenario, prePostContScen, dummyZ, Accuracy, &nodeObject[ gNodeID - 1 ], genParamFirstBase, genParamDZBase, genParamFirst, genParamSecondBase, genParamFirstCont, genParamFirstDZCont, genParamSecondCont, genParamCont, contingencyCount, genNumber ); // creates generatorInstance object with ID number i + 1
		...
		for ( int j = 0; j < loadNumber; ++j ) {
			...
			do {
				...
				//Power Consumption:
				if ((intervalID==0)||(intervalID==1)) 
					P_Load = matrixLoad[ j ][ 1 ] / divConvMWPU;
				else
					P_Load = matrixLoad[ j ][ 2 ] / divConvMWPU;			
	...
	// check the bounds and validity of the parameter values
	if (( prePostContScen == 0 ) && (postContScenario==0))
	...
	if (( prePostContScen != 0 ) && (postContScenario==0))
		deviceTermCount = genNumber + loadNumber + 2 * (translNumber-1); // total number of device-terminals
	if (( prePostContScen != 0 ) && (postContScenario!=0))
		deviceTermCount = genNumber + loadNumber + 2 * (translNumber-2); // total number of device-terminals
	...
	if (dummyZ==1) {
		if ( intervalID == 1 ) {
			for ( int i = 0; i < genNumber; ++i ) { 
				...
				for (int pcTrack = 0; pcTrack <= contingencyCount; ++pcTrack)
