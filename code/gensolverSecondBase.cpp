// solver.cpp
/* Produced by CVXGEN, 2019-02-05 19:37:47 -0500.  */
...
#include <cmath>
using namespace std;
double GensolverSecondBase::eval_gap(void) {
  int i;
  double gap;
  gap = 0;
  for (i = 0; i < 6; i++)
...
void GensolverSecondBase::set_defaults(void) {
...
void GensolverSecondBase::setup_pointers(void) {
  work.y = work.x + 3;
  work.s = work.x + 3;
  work.z = work.x + 9;
  vars.Pg = work.x + 0;
...
void GensolverSecondBase::setup_indexing(void) {
  setup_pointers();
}
void GensolverSecondBase::set_start(void) {
  int i;
  for (i = 0; i < 3; i++)
    work.x[i] = 0;
  for (i = 0; i < 0; i++)
...
double GensolverSecondBase::eval_objv(void) {
  int i;
  double objv;
...
void GensolverSecondBase::fillrhs_aff(void) {
  int i;
  double *r1, *r2, *r3, *r4;
  r1 = work.rhs;
  r2 = work.rhs + 3;
...
void GensolverSecondBase::fillrhs_cc(void) {
  int i;
  double *r2;
...
void GensolverSecondBase::refine(double *target, double *var) {
  int i, j;
...
double GensolverSecondBase::calc_ineq_resid_squared(void) {
...
double GensolverSecondBase::calc_eq_resid_squared(void) {
...
void GensolverSecondBase::better_start(void) {
...
void GensolverSecondBase::fillrhs_start(void) {
...
long GensolverSecondBase::solve(void) {
  int i;
  int iter;
  double *dx, *ds, *dy, *dz;
  double minval;
...
void GensolverSecondBase::tic(void) {
  tic_timestart = clock();
}
float GensolverSecondBase::toc(void) {
...
float GensolverSecondBase::tocq(void) {
  clock_t tic_timestop;
  tic_timestop = clock();
  return (float)(tic_timestop - tic_timestart) / CLOCKS_PER_SEC;
}
void GensolverSecondBase::printmatrix(char *name, double *A, int m, int n, int sparse) {
  int i, j;
  printf("%s = [...\n", name);
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++)
...
double GensolverSecondBase::unif(double lower, double upper) {
...
#define EPS 1.2e-7
#define RNMX (1.0-EPS)
float GensolverSecondBase::ran1(long*idum, int reset) {
  int j;
  long k;
  static long iy=0;
  static long iv[NTAB];
...
/* Next function is from numerical recipes in C. */
float GensolverSecondBase::randn_internal(long *idum, int reset) {
  static int iset=0;
  static float gset;
  float fac, rsq, v1, v2;
  if (reset) {
...
double GensolverSecondBase::randn(void) {
  return randn_internal(&global_seed, 0);
}
void GensolverSecondBase::reset_rand(void) {
  srand(15);
  global_seed = 1;
...
// matrix_support.cpp
/* Produced by CVXGEN, 2019-02-05 19:37:43 -0500.  */
/* CVXGEN is Copyright (C) 2006-2017 Jacob Mattingley, jem@cvxgen.com. */
/* The code in this file is Copyright (C) 2006-2017 Jacob Mattingley. */
/* CVXGEN, or solvers produced by CVXGEN, cannot be used for commercial */
/* applications without prior written permission from Jacob Mattingley. */
...
void GensolverSecondBase::multbymA(double *lhs, double *rhs) {
}
void GensolverSecondBase::multbymAT(double *lhs, double *rhs) {
  lhs[0] = 0;
  lhs[1] = 0;
  lhs[2] = 0;
}
void GensolverSecondBase::multbymG(double *lhs, double *rhs) {
  lhs[0] = -rhs[0]*(-1);
  lhs[1] = -rhs[0]*(1);
...
void GensolverSecondBase::multbymGT(double *lhs, double *rhs) {
  lhs[0] = -rhs[0]*(-1)-rhs[1]*(1)-rhs[2]*(params.selectZero[0])-rhs[3]*(-params.selectZero[0])-rhs[4]*(-1)-rhs[5]*(1);
  lhs[1] = -rhs[4]*(1)-rhs[5]*(-1);
  lhs[2] = 0;
}
void GensolverSecondBase::multbyP(double *lhs, double *rhs) {
...
void GensolverSecondBase::fillq(void) {
...
void GensolverSecondBase::fillh(void) {
...
void GensolverSecondBase::fillb(void) {
}
void GensolverSecondBase::pre_ops(void) {
...
void GensolverSecondBase::ldl_solve(double *target, double *var) {
  int i;
...
void GensolverSecondBase::ldl_factor(void) {
  work.d[0] = work.KKT[0];
  if (work.d[0] < 0)
...
double GensolverSecondBase::check_factorization(void) {
...
void GensolverSecondBase::matrix_multiply(double *result, double *source) {
...
double GensolverSecondBase::check_residual(double *target, double *multiplicand) {
...
void GensolverSecondBase::fill_KKT(void) {
...
/* Description: Basic test harness for solver.c. */
GensolverSecondBase::GensolverSecondBase( int numberOfCases, double cost2, double cost1, double cost0, double PgenMax, double PgenMin, double RgenMax, double RgenMin, double Beta, double InnerBeta, double Gamma, double innerGamma, int nextPChoice ) {
...
void GensolverSecondBase::load_default_data(int outerAPPIt, int APPItCount, double gsRho, double Pgenprev, double Pgenavg, double Powerprice, double Angpriceavg, double Angavg, double Angprice, double PgenAPP, double PgenAPPInner, double PgenPrevAPP, double AAPP, double BAPP, double LambAPP3, double LambAPP4, double BSC[], double lambdaSC[]) {
  if (outerAPPIt==1) {
   if (APPItCount==1) {
    params.rho[0] = gsRho;
...
double GensolverSecondBase::getPSol(void) {
 Piterate = (vars.Pg);
 return *Piterate;
}

double GensolverSecondBase::getPNextSol(void) {
 return params.PgNext[0];
}

double GensolverSecondBase::getPPrevSol(){ 
 PgPreviterate = (vars.PgPrev);
 return *PgPreviterate;
}

double GensolverSecondBase::getObj(void) {
 return (params.c2[0])*(*Piterate)*(*Piterate)+(params.c1[0])*(*Piterate)+(params.c0[0]);
}

double *GensolverSecondBase::getThetaPtr(void){
Thiterate = (vars.Thetag);
return Thiterate;
}

double GensolverSecondBase::getRMax() // start of getPMax function
{
	return params.RgMax[0];
} // end of getMax

double GensolverSecondBase::getRMin() // start of getPMin function
{
	return params.RgMin[0];
} // end of getPMin

double GensolverSecondBase::getPMax() // start of getPMax function
{
	return params.PgMax[0];
} // end of getMax

double GensolverSecondBase::getPMin() // start of getPMin function
{
	return params.PgMin[0];
} // end of getPMin

double GensolverSecondBase::getQuadCoeff() // start of getQuadCoeff function
{
	return params.c2[0]; 
} // end of getQuadCoeff

double GensolverSecondBase::getLinCoeff() // start of getLinCoeff function
{
	return params.c1[0]; 
} // end of getLinCoeff

double GensolverSecondBase::getConstCoeff() // start of getConstCoeff function
{
	return params.c0[0]; 
} // end of getConstCoeff

double GensolverSecondBase::getBeta(){return params.beta[0];}
double GensolverSecondBase::getIntBeta(){return params.betaInner[0];}
double GensolverSecondBase:::getGamma(){return params.gamma[0];}
double GensolverSecondBase::getIntGamma(){return params.gammaSC[0];}
int GensolverSecondBase::getLastIntChoice(){return params.selectZero[0];}
