...
//Workspace work;
//Settings settings;
GensolverFirst::GensolverFirst( int numberOfCases, double cost2, double cost1, double cost0, double PgenMax, double PgenMin, double RgenMax, double RgenMin, double Beta, double Gamma ) {
...
for ( int i = 0; i <= 10; ++i ) {
  params.ones[i] = 0.0;
  params.B[i] = -1.171028487447253;
  params.D[i] = -1.7941311867966805;
  params.lambda_1[i] = -0.23676062539745413;
  params.lambda_2[i] = -1.8804951564857322;
  params.PgNextNu[i] = 1.5851723557337523;
...
#define NUMTESTS 0
int GensolverFirst::main(int argc, char **argv) {
  int num_iters;
#if (NUMTESTS > 0)
  int i;
  double time;
...
void GensolverFirst::load_default_data(double gsRho, double Pgenprev[], double Pgenavg[], double Powerprice[], double Angpriceavg[], double Angavg[], double Angprice[]) {
  params.rho[0] = gsRho;
  params.PgNu[0] = 0.0;
for ( int i = 0; i <= params.caseNumber; ++i ) {
  params.ones[i] = 1.0;
  params.B[i] = 0.23541635196352795;
  params.D[i] = -0.44811496977740495;
  params.PgNextNu[i] = 1.5851723557337523;
  params.Pg_N_init[i] = Pgenprev[i];
  params.Pg_N_avg[i] = Pgenavg[i];
  params.lambda_1[i] = -1.0997819806406723;
  params.lambda_2[i] = -1.0997819806406723;
