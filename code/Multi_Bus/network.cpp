...
Network::Network( int val, int countOfScenario, int upcomingNext )
	: networkID( val ), // constructor begins; initialize networkID, Rho, scenarioIndex, nextUpcoming, and contingencyCount through constructor initializer list
	  Rho( 100.0 ),
	  scenarioIndex( countOfScenario ),
	  nextUpcoming( upcomingNext ),
	  contingencyCount( 0 )
{
...
// Function returning the total number of contingency scenarios
...
void Network::setNetworkVariables( int networkID ) // Function setNetworkVariables starts to initialize the parameters and variables
{
	...
	do {
		...
		if (( nextUpcoming == 1 ) && ( scenarioIndex != 0 ))
			contingencyCount -= 1; // reduce the contingency count by one to account for the remaining scenarios, in case of an outaged network scenario
		...
		int lineContingencyIndex = 0; // variable to keep track of the contingency scenario corresponding to which a line should be outaged in the txr line scenario copies
		int scenarioContInd = 0; // variable to keep track of the contingency scenario corresponding to which a line should be outaged in this coarse grain
		for ( int k = 0; k < translNumber; ++k ) {
			scenarioContInd = scenarioContInd + matrixTran[ k ][ 4 ];
			if ( ( nextUpcoming == 0 ) ||  ( ( nextUpcoming == 1 ) && ( ( scenarioIndex != scenarioContInd ) || ( ( scenarioIndex == scenarioContInd ) && ( matrixTran[ k ][ 4 ] == 0 ) ) ) ) ) {
		...
		for ( int i = 0; i < genNumber; ++i ) {
			...
			GensolverSecond genParamSecond( contingencyCount, c2, c1, c0, PgMax, PgMin, RgMax, RgMin, Beta, Gamma ); // Instantiate the copy constructor for the generator solver object
	
			Generator generatorInstance( i + 1, scenarioIndex, nextUpcoming, &nodeObject[ gNodeID - 1 ], genParamFirst, genParamSecond, contingencyCount ); // creates generatorInstance object with ID number i + 1
		...
		for ( int j = 0; j < loadNumber; ++j ) {
			...
			do {
				...
				//Power Consumption:
				P_Load = matrixLoad[ j ][ 1+nextUpcoming ] / divConvMWPU;
...
void Network::runSimulation(int countOfAPPIter, double appLambda[], double diffOfPow[] //Function runSimulation begins
{
	...
	string outputLogFileName = "resultInterval" + to_string(nextUpcoming) + "Scenario" + to_string(scenarioIndex) + ".txt";
	...
	for ( iteration_count = 1; ( iteration_count < 5001 ); iteration_count++ ) { // For LASCOPF, always use for loop, with a high, finite number of iterations
		...
		for ( generatorIterator = genObject.begin(); generatorIterator != genObject.end(); generatorIterator++ ) {
			...
			if ( solverChoice == 4 ) { // For CVXGEN Custom Solver
				...
				if ((nextUpcoming != 0) && (lastInterval == 0)) {
				...
				if ((nextUpcoming != 0) && (lastInterval == 1)) {
					double AAPP = -diffOfPow[2*(countOfDispInt-1)*genNumber+bufferIndex];
	...
	/**PRINT MW**/
	string outPowerResultFileName = "powerResultInterval" + to_string(nextUpcoming) + "Scenario" + to_string(scenarioIndex) + ".txt";
	...
	string outIterResultFileName = "itresultInterval" + to_string(nextUpcoming) + "Scenario" + to_string(scenarioIndex) + ".txt";
	...
	/**PRINT LMPs**/
	string outLMPResultFileName = "LMPresultInterval" + to_string(nextUpcoming) + "Scenario" + to_string(scenarioIndex) + ".txt";
	...
	string outObjResultFileName = "objectiveInterval" + to_string(nextUpcoming) + "Scenario" + to_string(scenarioIndex) + ".txt";
	...
	string outPrimResultFileName = "primresultInterval" + to_string(nextUpcoming) + "Scenario" + to_string(scenarioIndex) + ".txt";
	...
	string outDualResultFileName = "dualresultInterval" + to_string(nextUpcoming) + "Scenario" + to_string(scenarioIndex) + ".txt";
