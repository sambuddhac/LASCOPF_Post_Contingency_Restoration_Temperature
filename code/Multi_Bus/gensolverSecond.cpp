//solver.c
/* Produced by CVXGEN, 2016-05-03 19:36:33 -0400.  */
...
#include <cmath>
using namespace std;
double GensolverSecond::eval_gap(void) {
  int i;
  double gap;
  gap = 0;
  for (i = 0; i < 6; i++)
...
void GensolverSecond::set_defaults(void) {
...
void GensolverSecond::setup_pointers(void) {
  work.y = work.x + 13;
  work.s = work.x + 13;
  work.z = work.x + 19;
  vars.Pg = work.x + 0;
...
void GensolverSecond::setup_indexing(void) {
  setup_pointers();
}
void GensolverSecond::set_start(void) {
  int i;
  for (i = 0; i < 13; i++)
    work.x[i] = 0;
  for (i = 0; i < 0; i++)
...
double GensolverSecond::eval_objv(void) {
  int i;
  double objv;
...
void GensolverSecond::fillrhs_aff(void) {
  int i;
  double *r1, *r2, *r3, *r4;
  r1 = work.rhs;
  r2 = work.rhs + 13;
...
void GensolverSecond::fillrhs_cc(void) {
  int i;
  double *r2;
...
void GensolverSecond::refine(double *target, double *var) {
  int i, j;
...
double GensolverSecond::calc_ineq_resid_squared(void) {
...
double GensolverSecond::calc_eq_resid_squared(void) {
...
void GensolverSecond::better_start(void) {
...
void GensolverSecond::fillrhs_start(void) {
...
long GensolverSecond::solve(void) {
  int i;
  int iter;
  double *dx, *ds, *dy, *dz;
  double minval;
...
void GensolverSecond::tic(void) {
  tic_timestart = clock();
}
float GensolverSecond::toc(void) {
...
float GensolverSecond::tocq(void) {
  clock_t tic_timestop;
  tic_timestop = clock();
  return (float)(tic_timestop - tic_timestart) / CLOCKS_PER_SEC;
}
void GensolverSecond::printmatrix(char *name, double *A, int m, int n, int sparse) {
  int i, j;
  printf("%s = [...\n", name);
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++)
...
double GensolverSecond::unif(double lower, double upper) {
...
#define EPS 1.2e-7
#define RNMX (1.0-EPS)
float GensolverSecond::ran1(long*idum, int reset) {
  int j;
  long k;
  static long iy=0;
  static long iv[NTAB];
...
/* Next function is from numerical recipes in C. */
float GensolverSecond::randn_internal(long *idum, int reset) {
  static int iset=0;
  static float gset;
  float fac, rsq, v1, v2;
  if (reset) {
...
double GensolverSecond::randn(void) {
  return randn_internal(&global_seed, 0);
}
void GensolverSecond::reset_rand(void) {
  srand(15);
  global_seed = 1;
...
void GensolverSecond::multbymA(double *lhs, double *rhs) {
}
void GensolverSecond::multbymAT(double *lhs, double *rhs) {
  lhs[0] = 0;
  lhs[1] = 0;
  lhs[2] = 0;
  lhs[3] = 0;
...
void GensolverSecond::multbymG(double *lhs, double *rhs) {
  lhs[0] = -rhs[0]*(-1);
  lhs[1] = -rhs[0]*(1);
  lhs[2] = -rhs[0]*(1);
  lhs[3] = -rhs[0]*(-1);
...
void GensolverSecond::multbymGT(double *lhs, double *rhs) {
...
void GensolverSecond::multbyP(double *lhs, double *rhs) {
...
void GensolverSecond::fillq(void) {
...
void GensolverSecond::fillh(void) {
...
void GensolverSecond::fillb(void) {
}
void GensolverSecond::pre_ops(void) {
...
void GensolverSecond::ldl_solve(double *target, double *var) {
  int i;
...
void GensolverSecond::ldl_factor(void) {
  work.d[0] = work.KKT[0];
  if (work.d[0] < 0)
...
double GensolverSecond::check_factorization(void) {
...
void GensolverSecond::matrix_multiply(double *result, double *source) {
...
double GensolverSecond::check_residual(double *target, double *multiplicand) {
...
void GensolverSecond::fill_KKT(void) {
...
//Workspace work;
//Settings settings;
GensolverSecond::GensolverSecond( int numberOfCases, double cost2, double cost1, double cost0, double PgenMax, double PgenMin, double RgenMax, double RgenMin, double Beta, double Gamma ) {
...
GensolverSecond::~GensolverSecond() {
}
#define NUMTESTS 0
void GensolverSecond::mainsolve(int APPItCount, double gsRho, double Pgprev[], double Pgavrg[], double Pprice[], double Apriceavg[], double Aavg[], double Aprice[], double PgenPrevAPP, double PgenAPP, double AAPP, double BAPP, double LambAPP3, double LambAPP4) {
  int num_iters;
#if (NUMTESTS > 0)
  int i;
  double time;
...
void GensolverSecond::load_default_data(int APPItCount, double gsRho, double Pgenprev[], double Pgenavg[], double Powerprice[], double Angpriceavg[], double Angavg[], double Angprice[], double PgenPrevAPP, double PgenAPP, double AAPP, double BAPP, double LambAPP3, double LambAPP4) {
if (APPItCount==1) {
  params.rho[0] = gsRho;
  params.PgPrevNu[0] = 0.0;
  params.PgNu[0] = 0.0;
...
double GensolverSecond::getPSol(void) {
 Piterate = (vars.Pg);
 return *Piterate;
}

double GensolverSecond::getPPrevSol(void) {
 PgPreviterate = (vars.PgPrev);
 return *PgPreviterate;
}

double GensolverSecond::getPNextSol(void) {
 Piterate = (vars.PgNext);
 return *PgNextiterate;
}


double GensolverSecond::getObj(void) {
 return (params.c2[0])*(*Piterate)*(*Piterate)+(params.c1[0])*(*Piterate)+(params.c0[0]);
}

double *GensolverSecond::getThetaPtr(void){
Thiterate = (vars.Thetag);
return Thiterate;
}

double GensolverSecond::getRMax() // start of getPMax function
{
	return params.RgMax[0];
} // end of getMax

double GensolverSecond::getRMin() // start of getPMin function
{
	return params.RgMin[0];
} // end of getPMin

double GensolverSecond::getPMax() // start of getPMax function
{
	return params.PgMax[0];
} // end of getMax

double GensolverSecond::getPMin() // start of getPMin function
{
	return params.PgMin[0];
} // end of getPMin

double GensolverSecond::getQuadCoeff() // start of getQuadCoeff function
{
	return params.c2[0]; 
} // end of getQuadCoeff

double GensolverSecond::getLinCoeff() // start of getLinCoeff function
{
	return params.c1[0]; 
} // end of getLinCoeff

double GensolverSecond::getConstCoeff() // start of getConstCoeff function
{
	return params.c0[0]; 
