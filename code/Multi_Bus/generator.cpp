...
Generator::Generator( int idOfGen, int contScenarioCount, int upcomingNext, Node *nodeConng, GensolverFirst &paramOfGenFirst, GensolverSecond &paramOfGenSecond, int countOfContingency ) // constructor begins
	: genID( idOfGen ),
	  scenarioContCount( contScenarioCount ),
	  nextUpcoming( upcomingNext ),
	  connNodegPtr( nodeConng ),
	  genSolverFirst( paramOfGenFirst ),
	  genSolverSecond( paramOfGenSecond ),
...
{
	if ( nextUpcoming == 0 ) { // Use the solver for first dispatch interval
		...
		for ( int i = 0; i <= contCountGen; ++i ) {
			...
			PgNextFirst[ i ] = *( genSolverFirst.getPNextSol() + i ); // get the Generator Power iterate
			//*cout << "\nThiterate from generator: " << *(Thetag+i) << endl;
		}
	}
	else { // Use the solver for first dispatch interval
		genSolverSecond.mainsolve( APPIter, gRho, Pprevit, Pnetavg, uprev, vprevavg, Aprevavg, vprev, PgenPrevAPP, PgenAPP, AAPP, BAPP, LambAPP3, LambAPP4 ); // calls the Generator optimization solver
		Pg = genSolverSecond.getPSol(); // get the Generator Power iterate
		PgPrev = genSolverSecond.getPPrevSol(); // get the Generator Power iterate
		PgNext = genSolverSecond.getPNextSol(); // get the Generator Power iterate
		//Thetag = genSolver.getThetaPtr(); // get the Generator voltage angle iterate
		for ( int i = 0; i <= contCountGen; ++i ) {
			Thetag[ i ] = *( genSolverSecond.getThetaPtr() + i );
			//*cout << "\nThiterate from generator: " << *(Thetag+i) << endl;
		}
	}
